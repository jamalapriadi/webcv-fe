export const state = () => ({
    ...
    fields:[
        ...
        {key:'updated_at_human', label:'Date', sortable:false},
        {key: 'actions',label:'', sortable: false, thStyle:{width:'15%'}}, 
    ],
    addLink: '/portofolio/create',
    btnAction:true,
    edit_data_form: '/portofolio',
    backBtn:'/portofolio',
    lists:[],
    list:{},
    meta:{
...
export const actions = {
    async get_data({commit, state}){
        ...
        const res = await this.$repositories.post.perPage(params)
    ...
    async save({commit, state}, payload){
        ...
        try{
            const res = await this.$repositories.post.save(payload)
    ...
    async show({commit,state}, id){
        ...
        const res = await this.$repositories.post.show(id)
    ...
    async update({commit, state}, payload){
        ...
        try{
            const res = await this.$repositories.post.update(payload.kode, payload)
