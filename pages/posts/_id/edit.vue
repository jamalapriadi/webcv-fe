                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <client-only placeholder="loading...">
                                        <ckeditor-nuxt :config="editorConfig" v-model="form.desc"/>
                                    </client-only>
                                ...
                                <div id="collapse-2" class="accordion-collapse collapse" data-bs-parent="#accordion-example">
                                    <div class="accordion-body pt-0">
                                        <label class="form-check" v-for="(l,idx) in categories" :key="idx">
                                            <input class="form-check-input" type="checkbox" @input="selectCategory($event,l.id)" :checked="getChecked(l.id)">
                                ...
                                <div id="collapse-4" class="accordion-collapse collapse" data-bs-parent="#accordion-example">
                                    <div class="accordion-body pt-0">
                                        <label class="form-check">
                                            <input class="form-check-input" type="checkbox" @input="selectAllowComment($event)" :checked="[
                                                form.allow_comment == 'open' ? true : false
                                            ]">
...
export default {
    ...
    methods:{
        ...mapActions('post',['update','show','showCategory']),
        ...
        getData(){
            ...
            this.$axios.get('/auth/post/'+this.kode)
                .then(resp => {
                    this.form ={
                        kode: resp.data.data.id,
                        visibility:resp.data.data.visibility,
                        publish:resp.data.data.publish,
                        title: resp.data.data.title,
                        desc: resp.data.data.description,
                        featuredImage:{},
                        category:[],
                        allow_comment:resp.data.data.open_comment,
                        post_type:'artikel'
                    }

                    for(var a=0; a< resp.data.data.category.data.length; a++)
                    {
                        this.form.category.push(resp.data.data.category.data[a].id)

                        this.getChecked(resp.data.data.category.data[a].id)
                    }

                    if(resp.data.data.featured_image)
                    {
                        this.set_featured_image = 'Y'
                        this.form.featuredImage.file_url = resp.data.data.featured_image
                    }
                })
        },
