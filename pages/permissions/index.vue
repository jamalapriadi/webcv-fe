...
export default {
    ...
    validate({ params, query, store }) {
        ...
        {
            if(store.$auth.user.data.permissions[a].name == "list_permission")
            {
                return true
            }
        }
    ...
    async fetch({store}){
        await store.dispatch('permission/get_data')
    },
    computed:{
        ...mapState('permission',{
    ...
    methods:{
        ...mapActions('permission',['change_page','change_per_page','change_search_key','goToEdit','openDeleteModal']),
