...
export default {
    ...
    layout:'main',
    async fetch({store}){
        await store.dispatch('category/get_data')
    },
    validate({ params, query, store }) {
        for(var a=0;a<store.$auth.user.data.permissions.length;a++)
        {
            if(store.$auth.user.data.permissions[a].name == "list_category")
            {
                return true
            }
        }
    ...
    methods:{
        ...mapActions('category',['change_page','change_per_page','change_search_key','goToEdit','openDeleteModal']),
